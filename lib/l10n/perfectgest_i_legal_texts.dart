...
O PerfectGest I oferece controlo claro dos dados, em conformidade com a Google Play e a LGPD. A app é local-first.'''),
  ...
  LegalSectionText(heading: '1. Dados de gestão (local)', body: '''Armazenados em SQLite no dispositivo: clientes, orçamentos, caixa, ordens de serviço, dados da empresa (incluindo logótipo), cópia do cartão de visita, lembretes fiscais.
...
Backups partilhados: elimine manualmente no WhatsApp, e-mail ou nuvem onde os guardou.'''),
  LegalSectionText(heading: '2. Firebase / telemetria', body: '''Metadados anónimos de instalação e métricas de uso (elegibilidade de assinatura / anti-abuso). Não alojamos conteúdo do cartão de visita nem o logótipo em servidores.
...
const List<LegalSectionText> _kFaqPt = <LegalSectionText>[
  ...
  LegalSectionText(heading: 'Posso usar a app sem assinatura?', body: 'Sim, para explorar (navegar ecrãs e consultar a interface). Para operações (criar clientes, orçamentos ou caixa; gerar PDFs de gestão; restauro de backup encriptado; e demais acções condicionadas) é necessário o Plano Pro. Ao tentar uma operação sem assinatura, surge um aviso com o plano (referência R\$ 9,90/mês; preço na Google Play) e os botões Assinar ou Continuar a explorar recursos do Aplicativo.'),
  LegalSectionText(heading: 'Como funciona o Plano Pro?', body: 'Assinatura mensal ou anual na Google Play: uso ilimitado, restauro de backup encriptado e cartão de visita interactivo. Gerir em Configurações → Assinatura Google Play.'),
...
PerfectGest I provides clear data control under Google Play and LGPD. The app is local-first.'''),
  ...
  LegalSectionText(heading: '1. Business data (local)', body: '''Stored in SQLite on your device: clients, estimates, cash, work orders, company data (including logo), business card snapshot, fiscal reminders.
...
Shared backups: delete manually from WhatsApp, email, or cloud where you stored them.'''),
  LegalSectionText(heading: '2. Firebase telemetry', body: '''Anonymous install metadata and usage metrics (subscription eligibility / anti-abuse). We do not host business card content or the company logo on servers.
...
const List<LegalSectionText> _kFaqEn = <LegalSectionText>[
  ...
  LegalSectionText(heading: 'How does the Pro Plan work?', body: 'Monthly or annual subscription on Google Play: unlimited use, encrypted backup restore, and interactive business card. Manage in Settings → Google Play subscription.'),
...
PerfectGest I ofrece control claro de datos según Google Play y LGPD. La app es local-first.'''),
  ...
  LegalSectionText(heading: '1. Datos de gestión (local)', body: '''En SQLite en el dispositivo: clientes, presupuestos, caja, órdenes, empresa (incluido logotipo), snapshot de tarjeta, recordatorios fiscales.
...
Backups compartidos: elimínelos manualmente donde los guardó.'''),
  LegalSectionText(heading: '2. Firebase / telemetría', body: '''Metadatos anónimos de instalación (elegibilidad de suscripción / antiabuso). No alojamos contenido de tarjeta ni el logotipo en servidores.
...
const List<LegalSectionText> _kFaqEs = <LegalSectionText>[
  ...
  LegalSectionText(heading: '¿Cómo funciona el Plan Pro?', body: 'Suscripción mensual o anual en Google Play: uso ilimitado, restauración de backup cifrado y tarjeta interactiva. Gestión: Ajustes → Suscripción de Google Play.'),
