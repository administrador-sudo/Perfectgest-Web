((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,C,B={KL:function KL(d){this.a=d},
aWB(){return new B.nm(null)},
nm:function nm(d){this.a=d},
a0q:function a0q(){this.c=this.a=null},
avU:function avU(d,e,f,g,h){var _=this
_.a=d
_.b=e
_.c=f
_.d=g
_.e=h},
avT:function avT(){},
rz:function rz(d,e,f,g){var _=this
_.c=d
_.d=e
_.e=f
_.a=g},
Gf:function Gf(d,e,f){this.c=d
this.d=e
this.a=f},
aL7(d){switch(d.an(y.k).r.f.gcN()){case"en":return D.KN
case"es":return D.KO
case"pt":default:return D.KP}},
amM:function amM(){},
ayR:function ayR(){},
ayP:function ayP(){},
ayQ:function ayQ(){}},D,E,F,G
A=c[0]
C=c[2]
B=a.updateHolder(c[3],B)
D=c[13]
E=c[9]
F=c[12]
G=c[14]
B.KL.prototype={
K(d){var x,w=null,v=A.D(d),u=B.aL7(d)
v=v.ax.k3
x=A.b1().$3$color$fontSize$height(v.al(0.72),11.5,1.4)
return A.cc(A.b([A.a9("PERFECT GEST DESENVOLVIMENTO DE SOFTWARE LTDA",w,w,w,A.b1().$4$color$fontSize$fontWeight$height(v.al(0.85),12,C.aM,1.3),w,w),C.kr,A.a9(u.gKW()+" 66.889.409/0001-19",w,w,w,x,w,w),C.kq,A.a9(u.gKZ()+" +55 51 989045442",w,w,w,x,w,w),C.kq,A.a9("Caxias do Sul \u2014 RS \u2014 Brasil",w,w,w,x,w,w)],y.e),C.a7,C.C,C.E)}}
B.nm.prototype={
ag(){return new B.a0q()}}
B.a0q.prototype={
az(){this.aV()
E.aNZ()},
l(){A.x7()
this.aL()},
K(d){var x,w,v,u,t=null,s=A.D(d),r=A.bc(d,C.aQ,y.h).w.a.a,q=r<400?16:24,p=A.cw(d,C.aE,y.p)
p.toString
x=B.aL7(d)
w=x.ghm()
v=A.D(d).ax.a===C.I?C.p:t
u=x.gjT()
this.a.toString
return A.aX(t,t,A.nD(F.aGD(d,t,t,u),v,new A.lM(A.Ao(new B.avU(q,x,s.ax,r,p)),t),t),!1,t,t,t,!1,t,t,t,t,t,t,t,w,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,C.r,t)}}
B.rz.prototype={
K(d){var x,w,v,u,t,s=null,r=A.D(d).ax,q=A.bc(d,C.aQ,y.h).w,p=r.RG
p=(p==null?r.k2:p).al(0.55)
x=A.di(16)
w=r.ry
if(w==null){w=r.n
if(w==null)w=r.k3}w=A.kX(w.al(0.45),1)
q=q.a.a<400?14:16
v=r.k3
u=y.e
v=A.b([A.a9(this.c,s,s,s,A.b1().$3$color$fontSize$fontWeight(v,15,C.aN),s,s),C.cW,A.a9(this.d,s,s,s,A.b1().$3$color$fontSize$height(v.al(0.8),13.5,1.55),s,s)],u)
t=this.e
if(t!=null)C.b.a_(v,A.b([C.c0,A.m1(C.bt,t,C.cC,6,10)],u))
return new A.by(C.pW,A.em(new A.by(new A.af(q,q,q,q),A.cc(v,C.a7,C.C,C.E),s),new A.bU(p,s,w,x,s,s,C.Y),C.aA),s)}}
B.Gf.prototype={
K(d){var x=null
return A.it(A.a9(this.c,x,x,x,A.b1().$1$fontWeight(C.bq),x,x),this.d,x)}}
B.amM.prototype={}
B.ayR.prototype={
ghm(){return"Politica de devolucao e reembolso Perfect Gest Dev"},
gjT(){return"Devolucao e reembolso"},
gon(){return"Perfect Gest Dev"},
goI(){return"Pol\xedtica de devolu\xe7\xe3o e reembolso (site vitrine)"},
pb(d){return"\xdaltima atualiza\xe7\xe3o: documento institucional. Para solicitar reembolso, cancelamento ou esclarecimentos sobre o produto PerfectGest I, contacte: "+d+"."},
gnG(){return"1. Escopo (site vitrine)"},
gnF(){return"Este site \xe9 a vitrine institucional de Perfect Gest Dev (PERFECT GEST DESENVOLVIMENTO DE SOFTWARE LTDA), em https://perfectgestdev.com. As vendas e pagamentos do produto PerfectGest I n\xe3o s\xe3o processados neste site \u2014 ocorrem no canal de distribui\xe7\xe3o oficial do produto ou por acordo comercial directo connosco. Publicamos esta pol\xedtica em favor da transpar\xeancia com o nosso p\xfablico e utilizadores."},
gnI(){return"2. Direito de arrependimento (Brasil)"},
gnH(){return"Quando aplic\xe1vel a compras \xe0 dist\xe2ncia (CDC, art. 49), o consumidor pode exercer o direito de arrependimento em at\xe9 7 dias corridos. Se a compra n\xe3o ocorreu neste site, o pedido deve ser feito no canal onde adquiriu o produto e/ou pelo nosso contacto de suporte \u2014 estamos do lado do utilizador para orientar o processo."},
gnK(){return"3. Produtos digitais, assinaturas e cancelamento"},
gnJ(){return"Para produtos digitais e assinaturas, reembolso e cancelamento dependem do canal de aquisi\xe7\xe3o; cada canal tem fluxos e prazos pr\xf3prios. Independentemente do canal, pode contactar-nos: analisamos pedidos de forma imparcial e orientamos a melhor resolu\xe7\xe3o, incluindo falhas t\xe9cnicas comprovadas."},
gnM(){return"4. Como solicitar (passo a passo)"},
gnL(){return"1) Informe onde adquiriu o produto, data e comprovante.\n2) Descreva o motivo (arrependimento, cobran\xe7a indevida, defeito t\xe9cnico, etc.).\n3) Envie para o nosso e-mail de suporte.\nResponderemos com orienta\xe7\xf5es claras e, quando aplic\xe1vel, com o procedimento adequado ao seu caso."},
gKW(){return"CNPJ:"},
gKZ(){return"Contato com Empresa:"}}
B.ayP.prototype={
ghm(){return"Perfect Gest Dev returns and refunds policy"},
gjT(){return"Returns & refunds"},
gon(){return"Perfect Gest Dev"},
goI(){return"Returns & refunds policy (showcase website)"},
pb(d){return"Last updated: institutional document. To request a refund, cancellation, or clarification about PerfectGest I, contact: "+d+"."},
gnG(){return"1. Scope (showcase website)"},
gnF(){return"This website is the institutional showcase of Perfect Gest Dev (PERFECT GEST DESENVOLVIMENTO DE SOFTWARE LTDA) at https://perfectgestdev.com. Sales and payments for PerfectGest I are not processed on this site \u2014 they happen through the product's official distribution channel or a direct commercial agreement with us. We publish this policy for transparency with our audience and users."},
gnI(){return"2. Consumer withdrawal (Brazil)"},
gnH(){return"When applicable to remote purchases under Brazilian consumer law, a customer may exercise the withdrawal right within 7 calendar days. If the purchase did not happen on this website, the request must be made through the channel where you bought the product and/or via our support contact \u2014 we are on the user's side to guide the process."},
gnK(){return"3. Digital products, subscriptions, and cancellation"},
gnJ(){return"For digital products and subscriptions, refunds and cancellation depend on the purchase channel; each channel has its own flows and timelines. Regardless of channel, you may contact us: we review requests impartially and guide the best resolution, including confirmed technical issues."},
gnM(){return"4. How to request (step by step)"},
gnL(){return"1) State where you purchased the product, date, and proof.\n2) Describe the reason (withdrawal, incorrect charge, technical defect, etc.).\n3) Send it to our support email.\nWe will reply with clear guidance and, when applicable, the right procedure for your case."},
gKW(){return"CNPJ:"},
gKZ(){return"Company contact:"}}
B.ayQ.prototype={
ghm(){return"Pol\xedtica de devoluciones y reembolsos Perfect Gest Dev"},
gjT(){return"Devoluciones y reembolsos"},
gon(){return"Perfect Gest Dev"},
goI(){return"Pol\xedtica de devoluciones y reembolsos (sitio vitrina)"},
pb(d){return"\xdaltima actualizaci\xf3n: documento institucional. Para solicitar reembolso, cancelaci\xf3n o aclaraciones sobre PerfectGest I, contacte: "+d+"."},
gnG(){return"1. Alcance (sitio vitrina)"},
gnF(){return"Este sitio es la vitrina institucional de Perfect Gest Dev (PERFECT GEST DESENVOLVIMENTO DE SOFTWARE LTDA), en https://perfectgestdev.com. Las ventas y pagos del producto PerfectGest I no se procesan en este sitio \u2014 ocurren en el canal de distribuci\xf3n oficial del producto o por acuerdo comercial directo con nosotros. Publicamos esta pol\xedtica en favor de la transparencia con nuestro p\xfablico y usuarios."},
gnI(){return"2. Derecho de desistimiento (Brasil)"},
gnH(){return"Cuando aplique a compras a distancia seg\xfan la ley brasile\xf1a, el consumidor puede ejercer el derecho de desistimiento dentro de 7 d\xedas calendario. Si la compra no ocurri\xf3 en este sitio, la solicitud debe hacerse en el canal donde adquiri\xf3 el producto y/o por nuestro contacto de soporte \u2014 estamos del lado del usuario para orientar el proceso."},
gnK(){return"3. Productos digitales, suscripciones y cancelaci\xf3n"},
gnJ(){return"Para productos digitales y suscripciones, reembolso y cancelaci\xf3n dependen del canal de adquisici\xf3n; cada canal tiene flujos y plazos propios. Independientemente del canal, puede contactarnos: revisamos solicitudes de forma imparcial y orientamos la mejor resoluci\xf3n, incluidas fallas t\xe9cnicas comprobadas."},
gnM(){return"4. C\xf3mo solicitar (paso a paso)"},
gnL(){return"1) Indique d\xf3nde adquiri\xf3 el producto, fecha y comprobante.\n2) Describa el motivo (desistimiento, cobro indebido, defecto t\xe9cnico, etc.).\n3) Env\xedelo a nuestro correo de soporte.\nResponderemos con orientaciones claras y, cuando aplique, con el procedimiento adecuado a su caso."},
gKW(){return"CNPJ:"},
gKZ(){return"Contacto con la empresa:"}}
var z=a.updateTypes([])
B.avU.prototype={
$2(d,e){var x,w,v,u,t,s,r,q=this,p=null,o="suporte@perfectgestdev.com",n=e.b
n=n<1/0&&isFinite(n)?n:A.bc(d,C.aQ,y.h).w.a.a
x=C.d.dr(n<720?n:720,200,720)
w=q.a
v=q.b
u=q.c
t=A.a9(v.gon(),p,p,p,A.b1().$4$color$fontSize$fontWeight$letterSpacing(u.b,15,C.aN,0.4),p,p)
s=v.goI()
r=q.d<400?18:22
u=u.k3
u=A.b([t,C.ed,A.a9(s,p,p,p,A.b1().$4$color$fontSize$fontWeight$height(u,r,C.aN,1.2),p,p),C.c0,A.a9(v.pb(o),p,p,p,A.b1().$3$color$fontSize$height(u.al(0.72),13,1.45),p,p),C.hu,new B.rz(v.gnG(),v.gnF(),p,p),new B.rz(v.gnI(),v.gnH(),p,p),new B.rz(v.gnK(),v.gnJ(),p,p),new B.rz(v.gnM(),v.gnL(),A.b([new B.Gf(o,new B.avT(),p)],y.n),p),C.ec,D.Mz,G.Ht,A.a9(q.e.uI(A.qt(new A.eC(Date.now(),0,!1))),p,p,p,A.b1().$3$color$fontSize$fontWeight(u.al(0.7),12,C.bq),p,p)],y.e)
u.push(D.a8f)
return A.ko(A.eU(new A.dx(new A.aa(0,x,0,1/0),A.cc(u,C.a7,C.C,C.E),p),p,p),p,new A.af(w,16,w,28),C.af)},
$S:121}
B.avT.prototype={
$0(){return A.iH(A.dc("mailto:suporte@perfectgestdev.com",0,null),C.cc,null)},
$S:0};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(A.ag,[B.KL,B.rz,B.Gf])
w(B.nm,A.T)
w(B.a0q,A.X)
w(B.avU,A.oV)
w(B.avT,A.jI)
w(B.amM,A.I)
x(B.amM,[B.ayR,B.ayP,B.ayQ])})()
A.HQ(b.typeUniverse,JSON.parse('{"KL":{"ag":[],"e":[]},"Gf":{"ag":[],"e":[]},"nm":{"T":[],"e":[]},"a0q":{"X":["nm"]},"rz":{"ag":[],"e":[]}}'))
var y={p:A.a2("fx"),e:A.a2("r<e>"),n:A.a2("r<Gf>"),h:A.a2("fB"),k:A.a2("md")};(function constants(){D.KN=new B.ayP()
D.KO=new B.ayQ()
D.KP=new B.ayR()
D.Mz=new B.KL(null)
D.a8f=new A.dm(null,60,null,null)})()};
(a=>{a["ANPdAU23J+3U15lwsqM8PX34qhw="]=a.current})($__dart_deferred_initializers__);