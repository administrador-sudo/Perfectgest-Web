((a,b)=>{a[b]=a[b]||{}})(self,"$__dart_deferred_initializers__")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,B,C,D={
a9M(){var x=0,w=A.Q(y.f),v=1,u=[],t,s,r,q,p,o,n
var $async$a9M=A.M(function(d,e){if(d===1){u.push(e)
x=v}for(;;)switch(x){case 0:v=3
A.jC().$1("Acordando o servidor Render... aguarde.")
r=A.cY("https://onrender.com",0,null)
q=y.g
p=A.az(["Content-Type","application/json"],q,q)
x=6
return A.S(C.aOU(r,B.de.Km(A.az(["mensagem","Teste de iniciante com sucesso!","usuario","PerfectProAdmin","data_envio",new A.eE(Date.now(),0,!1).avZ()],q,q),null),p),$async$a9M)
case 6:t=e
if(t.b===200){A.jC().$1("Sucesso: o dado chegou no Elastic.")
r=t
A.jC().$1("ID do registro: "+A.j(J.jE(B.de.Yn(A.aOr(A.aNe(r.e)).fA(r.w),null),"id")))}else A.jC().$1("Erro do servidor: "+t.b)
v=1
x=5
break
case 3:v=2
n=u.pop()
s=A.ao(n)
A.jC().$1("Erro de conexao: verifique internet e endpoint.")
A.jC().$1("Detalhe do erro: "+A.j(s))
x=5
break
case 2:x=1
break
case 5:return A.O(null,w)
case 1:return A.N(u.at(-1),w)}})
return A.P($async$a9M,w)}}
J=c[1]
A=c[0]
B=c[2]
C=c[11]
D=a.updateHolder(c[9],D)
var z=a.updateTypes([])
var y={g:A.a1("o"),f:A.a1("~")}};
(a=>{a["gDK8BcoqlAuPkE2uOK/3JdtZq8Y="]=a.current})($__dart_deferred_initializers__);