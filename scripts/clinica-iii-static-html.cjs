#!/usr/bin/env node
/**
 * Gera HTML estático (preto/branco) para políticas PerfectGest-Clinica III.
 * Evita Flutter Web + tema escuro da vitrine (texto invisível).
 */
const fs = require('node:fs');
const path = require('node:path');

const DATA = require('./clinica-iii-legal-data.cjs');
...
function footerLinks(pageId, lang) {
  const L = DATA.labels[lang];
  const items = [
    { id: 'privacy', label: L.privacy, href: '/politica-privacidade-clinica-iii' },
    { id: 'terms', label: L.terms, href: '/termos-clinica-iii' },
    { id: 'health', label: L.health, href: '/dados-saude-lgpd-clinica-iii' },
  ];
  return items
    .map((item) => {
      if (item.id === pageId) {
  ...
  <script>
    (function () {
      ...
      function setLang(lang) {
        ...
        try { localStorage.setItem('pp_clinica_iii_lang', lang); } catch (e) {}
      }
      document.querySelectorAll('[data-set-lang]').forEach(function (btn) {
      ...
      var saved = null;
      try { saved = localStorage.getItem('pp_clinica_iii_lang'); } catch (e) {}
...
if (require.main === module) {
  ...
  console.log(`clinica-iii-static-html: ${slugs.length} paginas -> ${out}`);
}
