#!/usr/bin/env node
/**
 * Gera HTML estático (preto/branco) para políticas PerfectGest I no domínio.
 * Modelo Clinica III — independente da vitrine Flutter.
 */
  ...
  <script>
    (function () {
      ...
      function setLang(lang) {
        ...
        try { localStorage.setItem('pp_perfectgest_i_lang', lang); } catch (e) {}
      }
      document.querySelectorAll('[data-set-lang]').forEach(function (btn) {
      ...
      var saved = null;
      try { saved = localStorage.getItem('pp_perfectgest_i_lang'); } catch (e) {}
...
if (require.main === module) {
  ...
  console.log(`perfectgest-i-static-html: ${slugs.length} paginas -> ${out}`);
}
